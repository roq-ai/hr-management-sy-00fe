...
import { getPayrollInformations } from 'apiSdk/payroll-informations';
import { PayrollInformationInterface } from 'interfaces/payroll-information';

import { JobApplicationsListPage as ShowCaseList } from 'pages/job-applications';

function DashboardPage() {
  const { session } = useSession();

  const jobPostingsStat = useSWR<PaginatedInterface<JobPostingsInterface>>(
    () => `/job-postings`,
  ...
  const payrollInformationStat = useSWR<PaginatedInterface<PayrollInformationInterface>>(
    () => `/payroll-informations`,
    () => getPayrollInformations({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Job Postings',
      result: jobPostingsStat?.data?.totalCount,
      isLoading: jobPostingsStat.isLoading,
    ...
    {
      title: 'Payroll Informations',
      result: payrollInformationStat?.data?.totalCount,
      isLoading: payrollInformationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`job-applications`} passHref legacyBehavior>
                  <Button
