...
import { getInterviewsById, updateInterviewsById } from 'apiSdk/interviews';
...
function InterviewsEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<InterviewsInterface>(
  ...
  const handleSubmit = async (values: InterviewsInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateInterviewsById(id, values);
      mutate(updated);
      resetForm();
