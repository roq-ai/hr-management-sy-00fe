...
import { getJobPostings, deleteJobPostingsById } from 'apiSdk/job-postings';
import { JobPostingsInterface } from 'interfaces/job-postings';

type ColumnType = ColumnDef<JobPostingsInterface, unknown>;

interface JobPostingsListPageProps {
...
export function JobPostingsListPage(props: JobPostingsListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<JobPostingsInterface>>(
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteJobPostingsById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: JobPostingsInterface) => {
    if (hasAccess('job_postings', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              ...
              {hasAccess('job_postings', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
              ...
              {hasAccess('job_postings', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="job_postings" />
        </Flex>

        {hasAccess('job_postings', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
